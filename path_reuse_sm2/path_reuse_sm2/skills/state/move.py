        ...
        if not simulate_only:
            ...
            try:
                ...
                self.pr_node.get_logger().warn(
                    "[Move] 'pre_planned_trajectories' not in blackboard. This is expected in first execution."
                )
                pre_plans = {}
            except Exception as e:
                self.pr_node.get_logger().warn(f"[Move] Blackboard access failed: {e}. Continuing with empty dict.")
